...
class IncomingEmailTab(ttk.Frame):
    def __init__(self, parent, client: EmailClient):
        ...
        #self.incomingEmails = ttk.Frame(self, width=325)
        ...
        self.incomingEmailList = ttk.Combobox(self)
        self.incomingEmailList.grid(column=0, row=1, sticky='wnes')
        ...
        self.incomingEmailList.bind('<<ComboboxSelected>>', 
                lambda event, : self.updateButtonStatusOnClick())
        self.updateCategoryCombobox()
    
    def updateButtonStatusOnClick(self):
        positionOnServer = int(self.incomingEmailList.get().split(',')[0])
        self.onEmailButtonClick(positionOnServer)

    def updateUpcomingEmailList(self, category: str):
        category = self.categoryComboBox.get()
        emailListEntries = list()
        ...
        for email_status in email_status_list:
            ...
            self.emailPositions.append(positionOnServer)
            
            if isEmailRead == True:
                full_text = str(positionOnServer) + ',Read,' + preview
            else:
                full_text = str(positionOnServer) + ',Not read,' + preview
            emailListEntries.append(full_text)
            i += 1
        self.incomingEmailList['values'] = emailListEntries
        
        #Bind list box to change the email text whenever an entry is selected
        self.incomingEmailList.bind('<<ComboboxSelected>>', 
                lambda event, : self.updateButtonStatusOnClick())
...
class FilterConfigTab(ttk.Frame):
    def __init__(self, parent, client: EmailClient):
        ...
        self.filterList = ttk.Combobox(self)
        self.filterList.grid(column=0, row=0, sticky='wnes', padx=5, pady=5)


        self.filterActionButtons = ttk.Frame(self)
        self.filterActionButtons.grid(column=2, row=5, sticky='wnes', padx=5, pady=5)
    ...
    def updateFilterList(self): 
        ...
        self.filterList['values'] = filter_categories
        self.filterList.bind('<<ComboboxSelected>>', lambda event: self.onFilterButtonClick())
        
    
    def onFilterButtonClick(self):
        category = self.filterList.get()
...
class GeneralConfigTab(ttk.Frame):
    ...
    def loadConfig(self):
        ...
        setEntryWidgetText(self.usernameText, config['Username'])
        setEntryWidgetText(self.passwordText, config['Password'])
        setEntryWidgetText(self.emailAddressText, config['Address'])
        setEntryWidgetText(self.serverText, config['Server'])
        setEntryWidgetText(self.SMTPPortText, config['SMTP Port'])
        setEntryWidgetText(self.POP3PortText, config['POP3 Port'])
        setEntryWidgetText(self.autoloadText, config['Autoload']) 
